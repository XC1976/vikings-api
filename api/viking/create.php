...
if (validateMandatoryParams($data, ['name', 'health', 'attack', 'defense', 'weaponID'])) {
    verifyViking($data);
    verifyWeaponIDError($data);

    $newVikingId = createViking($data['name'], $data['health'], $data['attack'], $data['defense'], $data['weaponID']);
    if (!$newVikingId) {
    ...
    returnError(412, 'Mandatory parameters : name, health, attack, defense, weaponID');
}